...
export function RecipeCard({ recipe, index }: RecipeCardProps) {
  ...
  return (
    <motion.div
      className="bg-white rounded-2xl shadow-sm border border-terracotta-50 overflow-hidden flex flex-col"
      initial={{ opacity: 0, y: 30 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4, delay: index * 0.1, ease: "easeOut" }}
    >
      {!imgError && (
        <div className="h-48 overflow-hidden bg-terracotta-50">
          <img
            src={`/images/${recipe.image_name}`}
            alt={recipe.title}
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          ...
          {Object.entries(recipe.explanation.constraints_met).map(([key, val]) => (
            <span
              key={key}
